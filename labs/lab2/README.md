...
1. Все слова не зависимые и распределенны равномерно.
2. Известно распределение `p(w_i|w_{i-1}) = Dir(mu(w_{i-1}))` где `Dir` является распределением Дирихле с вектором параметров `mu(w_i, w_{i-1})` размерности `d`. Причем компоненты вектора `mu(w_{i-1})` задаются выражением: `mu_i = exp(-|i-j|)`, где j это номер слова `w_{i-1}`.
